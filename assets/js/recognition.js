...
if (roleId != 3) {
  $("#btnCreate").removeClass("d-none").show();
}

document.addEventListener('DOMContentLoaded', function () {
  showSelectDepartment();

  showAllRecognition(selectedDepartment, selectedStatus, selectedType);
  showRecognitionCreated(selectedStatus1, selectedType1);
});
...
async function showAllRecognition(departmentId, statusId, type) {
  try {
    showLoadingOverlay();
    ...
    const response = await fetch(API.Recognition.GET_ALL + param, {
      method: "GET",
      mode: 'cors',
      headers: {
        "Content-Type": "application/json",
...
function showData(data) {
  ...
  const table = document.getElementById('table-recognition');
  ...
  const processedData = data.map(item => [
    item.employeeCode,
    item.departmentName,
    item.type ? `<div class="text-success"><b>Bonus</b></div>` : `<div class="text-danger"><b>Deduction</b></div>`,
    item.amount != null ? formatMoney(item.amount) : '',
    DateTime.fromFormat(item.createdDate, 'yyyy-MM-dd HH:mm:ss').toFormat('MM/dd/yyyy'),
    item.date ? DateTime.fromFormat(item.date, 'yyyy-MM-dd HH:mm:ss').toFormat('MM/dd/yyyy') : '',
    `<div class="${statusColors[item.statusId - 6]}"><b>${item.statusName}</b></div>`,
    `<button type="button" class="btn btn-info btn-sm text-white" title="View" data-recognition-id="${item.id}" onclick="viewOnClick(${item.id})">
          <i class="bi bi-eye"></i>
    </button>`
  ]);
  ...
  dataTable = new simpleDatatables.DataTable('#table-recognition', {
    data: {
      headings: headings,
      data: processedData
    },
    ...
    labels: {
      ...
      pageTitle: "Page {page}",
      perPage: "recognitions per page",
      noRows: "No recognitions found",
      info: "Showing {start} to {end} of {rows} recognitions",
...
async function showRecognitionCreated(statusId, type) {
  try {
    showLoadingOverlay();
    ...
    const response = await fetch(API.Recognition.GET_MY_CREATE + param, {
      method: "GET",
      mode: 'cors',
      headers: {
        "Content-Type": "application/json",
...
function showData1(data) {
  // Extract headings from the table
  const table = document.getElementById('table-recognition1');
  const headings = Array.from(table.querySelectorAll('thead th')).map(th => th.textContent);
  var statusColors = ['text-warning', 'text-success', 'text-danger'];
  ...
  const processedData = data.map(item => [
    ...
    item.date ? DateTime.fromFormat(item.date, 'yyyy-MM-dd HH:mm:ss').toFormat('MM/dd/yyyy') : '',
    `<div class="${statusColors[item.statusId - 6]}"><b>${item.statusName}</b></div>`,
    `<button type="button" class="btn btn-info btn-sm text-white" title="View" data-recognition1-id="${item.id}" onclick="viewOnClick(${item.id})">
          <i class="bi bi-eye"></i>
    </button>`
  ]);
  ...
  // Initialize DataTable
  dataTable1 = new simpleDatatables.DataTable('#table-recognition1', {
    data: {
      headings: headings,
      data: processedData
    },
  ...
  const inputTypeElement = document.getElementById('inputType1');
