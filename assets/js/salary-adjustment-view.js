...
const salaryAdjustmentId = localStorage.getItem("salaryAdjustmentId");

$("#deleteButton").hide();

document.addEventListener('DOMContentLoaded', function () {
    showSalaryAdjustment(salaryAdjustmentId);

    $("#updateButton").on("click", async function () {        
        const result = await Swal.fire({
            title: 'Update this salary adjustment',
            icon: 'warning',
        ...
        if (result.isConfirmed) {            
            ...
            updateSalaryAdjustment(salaryAdjustmentId, dataInput);
    ...
    document.getElementById('deleteButton').addEventListener('click', function () {
        deleteSalaryAdjustment(salaryAdjustmentId);
    });
});

async function showSalaryAdjustment(salaryAdjustmentId) {
    try {
        showLoadingOverlay();

        const response = await fetch(API.SalaryAdjustment.GET_BY_ID.replace("id", salaryAdjustmentId), {
            method: "GET",
            mode: 'cors',
            headers: {
...
function showData(data) {
    ...
    document.getElementById("inputOldSalary").value = formatMoney(data.oldSalary);
    document.getElementById("inputNewSalary").value = formatMoney(data.newSalary);
    ...
    document.getElementById("inputModifiedDate").value = data.statusId != 6 ?
        DateTime.fromFormat(data.lastModifiedDate, 'yyyy-MM-dd HH:mm:ss').toFormat('MM/dd/yyyy HH:mm:ss') : '';
...
async function updateSalaryAdjustment(salaryAdjustmentId, dataInput) {
    try {
        const response = await fetch(API.SalaryAdjustment.UPDATE.replace("id", salaryAdjustmentId), {
            method: "PATCH",
        ...
        if (response.ok) {
            toastr.success("Update salary adjustment successfully!");
            setTimeout(function () {
...
function deleteSalaryAdjustment(salaryAdjustmentId) {
    Swal.fire({
        ...
        if (result.isConfirmed) {
            try {
                ...
                const response = await fetch(API.SalaryAdjustment.DELETE.replace("id", salaryAdjustmentId), {
                    method: "DELETE",
                    mode: 'cors',
                    headers: {
                        "Content-Type": "application/json",
                ...
                if (response.ok) {
                    ...
                    setTimeout(function () {
                        location.href = "../salary-adjustment.html";
                    }, 1500);
                } else {
                    const errorResponse = await response.json();
